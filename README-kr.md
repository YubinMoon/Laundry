...
| 데이터 범위           | 출처                                                                    | 사용 목적                                |
| --------------------- | ----------------------------------------------------------------------- | ---------------------------------------- |
| 상의/하의 원천 이미지 | [DeepFashion2 Dataset](https://github.com/switchablenorms/DeepFashion2) | 대분류 및 상의/하의 소분류 데이터셋 구성 |
| 수건 이미지           | [Roboflow Universe](https://universe.roboflow.com/)                     | 대분류 모델의 `towel` 클래스 구성        |
| 양말 이미지           | [Roboflow Universe](https://universe.roboflow.com/)                     | 대분류 모델의 `socks` 클래스 구성        |

<!-- TODO: DeepFashion2 라이선스 정보를 입력하세요. -->
...
대분류 모델은 `YOLO26n`을 사용해 세탁물을 화면에서 찾고, 감지된 물체를 앱의 상위 카테고리로 바로 매핑하기 위해 `top`, `bottom`, `socks`, `towel` 4개 라벨로 학습했습니다. `top`과 `bottom` 데이터는 CLIP으로 세부 타입 선별을 통과한 crop 이미지의 원본 이미지와 해당 bbox label에서 구성했고, 여기에 수건과 양말 데이터를 추가했습니다.
...
| 양말        | `socks`   | 8k            | Roboflow Universe에서 가져온 양말 데이터셋을 `socks`로 구성    |
| 수건        | `towel`   | 3k            | Roboflow Universe에서 가져온 수건 데이터셋을 그대로 사용       |
...
상의와 하의 소분류 모델은 DeepFashion2 원본 데이터에 기록된 bbox를 crop한 이미지에 CLIP 기반 세부 클래스 선별을 적용해 `YOLO26n-cls` 학습 데이터셋으로 구성했습니다. CLIP은 별도 데이터 출처가 아니라, crop 이미지를 세부 의류 클래스로 나누고 학습에 사용할 품질 좋은 이미지를 고르기 위한 분류 도구로 사용했습니다. 앱에서도 대분류 모델이 감지한 영역을 crop한 뒤 해당 crop 이미지를 소분류 모델 입력으로 사용합니다.
...
4. 목표 세부 클래스별 텍스트 프롬프트를 정의하고, CLIP으로 crop 이미지와 프롬프트 간 유사도를 계산합니다.
...
<!-- TODO: 사용한 CLIP 모델명, 프롬프트 예시, 수동 검수 기준, 제거한 crop 이미지 수를 입력하세요. -->
...
<!-- TODO: 그룹별 세탁 주기, 추천 우선순위 기준, 그룹 완료 처리 정책을 더 자세히 설명하세요. -->
